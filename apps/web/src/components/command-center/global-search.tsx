...
export function GlobalSearch({ workspace, onClose, onNavigate }: Props) {
  ...
  const results = useMemo(() => {
    ...
    if (!value) return [];
    return [
      ...workspace.notes.filter((item) => (item.status ?? "active") !== "trashed" && `${item.title} ${item.content} ${(item.tags ?? []).join(" ")} ${item.folder ?? ""} ${(item.attachments ?? []).map((attachment) => attachment.extractedText ?? "").join(" ")}`.toLocaleLowerCase("id-ID").includes(value)).map((item) => ({ id: item.id, title: item.title, detail: `Note · ${item.folder || "tanpa folder"}`, icon: NotebookPen, section: "notes" as const })),
      ...workspace.transactions.filter((item) => `${item.payee ?? ""} ${item.note} ${item.date} ${(item.receiptAttachments ?? []).map((attachment) => attachment.extractedText ?? "").join(" ")}`.toLocaleLowerCase("id-ID").includes(value)).map((item) => ({ id: item.id, title: item.payee || item.note || "Transaksi", detail: item.date, icon: ReceiptText, section: "transaksi" as const })),
