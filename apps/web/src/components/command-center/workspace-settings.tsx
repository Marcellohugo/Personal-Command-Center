...
export function WorkspaceSettings({ workspace, updateWorkspace, syncState, conflict, onUseRemote, onKeepLocal, onEnableNotifications, onResetWorkspace }: Props) {
  ...
  return <section className="grid gap-5">
    ...
    <section className="panel grid gap-4 rounded-xl p-5"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-moss/10 text-moss"><ShieldCheck className="h-5 w-5" /></span><div><h3 className="font-black">Anggaran & pengingat</h3><p className="mt-1 text-sm text-ink/55 dark:text-paper/50">Atur batas pengeluaran dan waktu ritual pribadi.</p></div></div><label className="grid gap-2"><span className="label">Anggaran bulanan</span><input className="field" type="number" min="0" step="100000" value={workspace.settings.monthlyBudget || ""} onChange={(event) => updateWorkspace((current) => ({ ...current, settings: { ...current.settings, monthlyBudget: Number(event.target.value) || 0 } }))} placeholder="0" /></label><div className="grid gap-3 sm:grid-cols-3"><label className="grid gap-2"><span className="label">Ritual pagi</span><input className="field" type="time" value={workspace.settings.morningReminder ?? "08:00"} onChange={(event) => updateWorkspace((current) => ({ ...current, settings: { ...current.settings, morningReminder: event.target.value } }))} /></label><label className="grid gap-2"><span className="label">Ritual malam</span><input className="field" type="time" value={workspace.settings.eveningReminder ?? "20:00"} onChange={(event) => updateWorkspace((current) => ({ ...current, settings: { ...current.settings, eveningReminder: event.target.value } }))} /></label><label className="grid gap-2"><span className="label">Review mingguan</span><input className="field" type="time" value={workspace.settings.weeklyReviewReminder ?? "18:00"} onChange={(event) => updateWorkspace((current) => ({ ...current, settings: { ...current.settings, weeklyReviewReminder: event.target.value } }))} /></label></div><button type="button" className="button-secondary w-fit" onClick={onEnableNotifications}>Aktifkan pengingat lokal</button><p className="text-xs text-ink/45 dark:text-paper/45">Pengingat lokal berjalan ketika aplikasi tersedia; sinkronisasi push dipakai saat perangkat online.</p></section>
