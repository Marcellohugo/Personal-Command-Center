...
import { CalendarDays, Download, Eye, EyeOff, KeyRound, RefreshCw, ShieldCheck, Trash2, Upload } from "lucide-react";
...
export function WorkspaceSettings({ workspace, updateWorkspace, syncState, conflict, onUseRemote, onKeepLocal, onEnableNotifications, onResetWorkspace }: Props) {
  ...
  function resizePaydays(count: number) {
    updateWorkspace((current) => ({
      ...current,
      settings: {
        ...current.settings,
        paydays: Array.from({ length: count }, (_, index) => current.settings.paydays[index] ?? { day: 1, amount: 0 })
      }
    }));
  }

  function updatePayday(index: number, field: "day" | "amount", value: number) {
    updateWorkspace((current) => ({
      ...current,
      settings: {
        ...current.settings,
        paydays: current.settings.paydays.map((payday, paydayIndex) => paydayIndex === index
          ? { ...payday, [field]: field === "day" ? Math.max(1, Math.min(31, Math.round(value) || 1)) : Math.max(0, value || 0) }
          : payday)
      }
    }));
  }
  ...
  return <section className="grid gap-5">
    ...
    <section className="panel grid gap-4 rounded-xl p-5">
      <div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-600/10 text-blue-700 dark:text-blue-300"><CalendarDays className="h-5 w-5" /></span><div><h3 className="font-black">Jadwal gajian</h3><p className="mt-1 text-sm text-ink/55 dark:text-paper/50">Dipakai untuk membagi tagihan, cicilan, hutang, dan tabungan ke setiap gajian.</p></div></div>
      <label className="grid max-w-xs gap-2"><span className="label">Berapa kali gajian per bulan?</span><select className="field" value={workspace.settings.paydays.length} onChange={(event) => resizePaydays(Number(event.target.value))}><option value={0}>Belum diatur</option><option value={1}>1 kali</option><option value={2}>2 kali</option><option value={3}>3 kali</option><option value={4}>4 kali</option></select></label>
      {workspace.settings.paydays.length ? <div className="grid gap-3 sm:grid-cols-2">{workspace.settings.paydays.map((payday, index) => <div className="grid grid-cols-2 gap-3 rounded-xl border border-line bg-blue-50/50 p-3 dark:border-white/10 dark:bg-blue-400/5" key={index}><p className="col-span-2 text-sm font-black text-blue-800 dark:text-blue-200">Gajian {index + 1}</p><label className="grid gap-1"><span className="label">Tanggal</span><input className="field" type="number" min={1} max={31} value={payday.day} onChange={(event) => updatePayday(index, "day", Number(event.target.value))} /></label><label className="grid gap-1"><span className="label">Gaji bersih</span><input className="field" type="number" min={0} step={10000} value={payday.amount || ""} placeholder="0" onChange={(event) => updatePayday(index, "amount", Number(event.target.value))} /></label></div>)}</div> : <p className="text-sm text-ink/45 dark:text-paper/45">Atur jumlah gajian agar peta pembayaran otomatis muncul di Pusat Keuangan.</p>}
    </section>
